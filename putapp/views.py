...
def put_TwoStep_Binomial(request):
    ...
    c0 = (max(k0 - s0 * pow(su, 2), 0) * pow(p, 2) + max(k0 - s0 * pow(sd, 2), 0) * pow(1 - p, 2) + 2 * max(
        k0 - s0 * su * sd, 0) * p * (1 - p)) * pow(e, -r * t0)
    return render(request, 'output.html', {'result': c0})


def put_nStep_binomial(request):
    ...
    for j in range(0, n + 1):
        ...
        c = a / b
        c0 = c0 + c * pow(p, j) * pow(1 - p, n - j) * max(k0 - s0 * pow(su, j) * pow(sd, n - j), 0)
...
def put_Black_Scholes(request):
    ...
    price = k0 * np.exp(-r * t0) * norm.cdf(-d2, 0, 1) - s0 * norm.cdf(-d1, 0, 1)
